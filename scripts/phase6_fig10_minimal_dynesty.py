from __future__ import annotations

import argparse
from pathlib import Path

import dynesty
import matplotlib.lines as mlines
import matplotlib.pyplot as plt
import numpy as np
from dynesty.utils import resample_equal

from ringdown.fd_likelihood import (
    aligo_zero_det_high_power_psd,
    complex_ringdown_mode_tilde,
    continuous_ft_from_time_series,
    draw_colored_noise_rfft,
    optimal_snr,
)
from ringdown.frequencies import kerr_qnm_omegas_22n
from ringdown.preprocess import align_to_peak
from ringdown.sxs_io import load_sxs_waveform22


MSUN_SEC = 4.92549095e-6


def parse_n_values(text: str) -> list[int]:
    vals = [int(s.strip()) for s in text.split(",") if s.strip()]
    if not vals:
        raise ValueError("empty --n-values")
    return sorted(set(vals))


def credible_level_2d(pdf: np.ndarray, cred: float) -> float:
    flat = pdf.ravel()
    order = np.argsort(flat)[::-1]
    cdf = np.cumsum(flat[order])
    idx = int(np.searchsorted(cdf, cred, side="left"))
    idx = min(max(idx, 0), order.size - 1)
    return float(flat[order[idx]])


def normalize_1d_pdf(x: np.ndarray, p: np.ndarray) -> np.ndarray:
    z = float(np.trapezoid(p, x))
    return p / z if z > 0 else p


def n_tagged_path(base: Path, n: int) -> Path:
    suffix = base.suffix if base.suffix else ".npz"
    stem = base.stem if base.suffix else str(base)
    return base.parent / f"{Path(stem).stem}_N{n}{suffix}"


class RingdownInferenceModel:
    def __init__(
        self,
        *,
        freqs_hz: np.ndarray,
        d_tilde: np.ndarray,
        psd: np.ndarray,
        duration_sec: float,
        h_peak: float,
        n_overtones: int,
        m_total_msun: float,
        mf_bounds: tuple[float, float],
        chif_bounds: tuple[float, float],
        amp_bounds_rel: tuple[float, float],
        phi_bounds: tuple[float, float],
        qnm_chi_grid_size: int,
    ) -> None:
        self.n_overtones = int(n_overtones)
        self.n_modes = self.n_overtones + 1
        self.ndim = 2 + 2 * self.n_modes
        self.h_peak = float(h_peak)
        self.m_total_msun = float(m_total_msun)
        self.m_sec = MSUN_SEC * self.m_total_msun
        self.duration_sec = float(duration_sec)

        self.mf_bounds = mf_bounds
        self.chif_bounds = chif_bounds
        self.amp_bounds_rel = amp_bounds_rel
        self.phi_bounds = phi_bounds

        valid = (psd > 0.0) & np.isfinite(psd) & (freqs_hz > 0.0)
        if np.count_nonzero(valid) < 16:
            raise ValueError("too few valid frequency bins")
        self.f_calc = freqs_hz[valid]
        self.d_calc = d_tilde[valid]
        self.psd_calc = psd[valid]
        self.df = float(self.f_calc[1] - self.f_calc[0])
        self.d_weighted = self.d_calc / self.psd_calc

        self.d_d_inner = float(4.0 * self.df * np.sum((np.abs(self.d_calc) ** 2) / self.psd_calc))

        # Cache Kerr (2,2,n) frequencies on a chi grid at mf=1 and interpolate in likelihood.
        # This keeps the physics identical while removing repeated costly qnm solves.
        if qnm_chi_grid_size < 32:
            raise ValueError("qnm_chi_grid_size must be >= 32")
        chi_lo = max(self.chif_bounds[0], 0.0)
        chi_hi = min(self.chif_bounds[1], 0.999)
        self._chi_interp = np.linspace(chi_lo, chi_hi, int(qnm_chi_grid_size))
        self._qnm_re = np.empty((self.n_modes, self._chi_interp.size), dtype=float)
        self._qnm_im = np.empty((self.n_modes, self._chi_interp.size), dtype=float)
        for j, c in enumerate(self._chi_interp):
            vals = kerr_qnm_omegas_22n(mf=1.0, chif=float(c), n_max=self.n_overtones)
            self._qnm_re[:, j] = vals.real
            self._qnm_im[:, j] = vals.imag

    def prior_transform(self, u: np.ndarray) -> np.ndarray:
        theta = np.zeros_like(u)
        theta[0] = self.mf_bounds[0] + (self.mf_bounds[1] - self.mf_bounds[0]) * u[0]
        theta[1] = self.chif_bounds[0] + (self.chif_bounds[1] - self.chif_bounds[0]) * u[1]
        for n in range(self.n_modes):
            ia = 2 + 2 * n
            ip = 3 + 2 * n
            theta[ia] = self.h_peak * (
                self.amp_bounds_rel[0]
                + (self.amp_bounds_rel[1] - self.amp_bounds_rel[0]) * u[ia]
            )
            theta[ip] = self.phi_bounds[0] + (self.phi_bounds[1] - self.phi_bounds[0]) * u[ip]
        return theta

    def log_likelihood(self, theta: np.ndarray) -> float:
        mf_msun = float(theta[0])
        chif = float(theta[1])
        if not (self.mf_bounds[0] <= mf_msun <= self.mf_bounds[1]):
            return -1e300
        if not (self.chif_bounds[0] <= chif <= self.chif_bounds[1]):
            return -1e300

        amplitudes = theta[2::2]
        phases = theta[3::2]
        if np.any(amplitudes <= 0.0):
            return -1e300

        mf_frac = mf_msun / self.m_total_msun
        if mf_frac <= 0.0:
            return -1e300

        omegas_m = np.empty(self.n_modes, dtype=complex)
        for k in range(self.n_modes):
            wr = np.interp(chif, self._chi_interp, self._qnm_re[k])
            wi = np.interp(chif, self._chi_interp, self._qnm_im[k])
            omegas_m[k] = (wr + 1j * wi) / mf_frac
        omegas_rad_s = omegas_m / self.m_sec

        h_tilde = complex_ringdown_mode_tilde(
            self.f_calc,
            omegas_rad_s,
            amplitudes,
            phases,
            duration_sec=self.duration_sec,
        )

        d_h_inner = 4.0 * self.df * np.sum(np.real(self.d_weighted * np.conjugate(h_tilde)))
        h_h_inner = 4.0 * self.df * np.sum((np.abs(h_tilde) ** 2) / self.psd_calc)
        return float(d_h_inner - 0.5 * h_h_inner)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--sxs-location", type=str, default="SXS:BBH:0305v2.0/Lev6")
    p.add_argument("--no-download", action="store_true")
    p.add_argument("--n-values", type=str, default="3")
    p.add_argument("--m-total-msun", type=float, default=72.0)
    p.add_argument("--distance-mpc", type=float, default=400.0)
    p.add_argument("--target-hpeak", type=float, default=2e-21)
    p.add_argument("--target-snr-postpeak", type=float, default=42.3)
    p.add_argument("--snr-rtol", type=float, default=0.05)
    p.add_argument("--delta-t0-ms", type=float, default=0.0)
    p.add_argument("--t-end", type=float, default=90.0)
    p.add_argument("--f-min-hz", type=float, default=20.0)
    p.add_argument("--f-max-hz", type=float, default=1024.0)
    p.add_argument("--df-hz", type=float, default=1.0)
    p.add_argument("--mf-min-msun", type=float, default=50.0)
    p.add_argument("--mf-max-msun", type=float, default=100.0)
    p.add_argument("--chif-min", type=float, default=0.0)
    p.add_argument("--chif-max", type=float, default=0.999)
    p.add_argument("--amp-min-rel", type=float, default=0.01)
    p.add_argument("--amp-max-rel", type=float, default=250.0)
    p.add_argument("--nlive-init", type=int, default=800)
    p.add_argument("--nlive-batch", type=int, default=400)
    p.add_argument("--dlogz-init", type=float, default=0.2)
    p.add_argument("--maxbatch", type=int, default=8)
    p.add_argument("--maxcall", type=int, default=0)
    p.add_argument("--n-effective", type=float, default=0.0)
    p.add_argument("--sample", choices=["auto", "unif", "rwalk", "slice", "rslice", "hslice"], default="rslice")
    p.add_argument("--walks", type=int, default=64)
    p.add_argument("--slices", type=int, default=12)
    p.add_argument("--qnm-chi-grid-size", type=int, default=240)
    p.add_argument(
        "--enable-constant-bias",
        action="store_true",
        help="Add a constant real bias term to the observed data in time domain before FFT.",
    )
    p.add_argument(
        "--constant-bias-rel",
        type=float,
        default=0.0,
        help="Constant bias amplitude relative to h_peak, used when --enable-constant-bias is set.",
    )
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--output", type=Path, default=Path("results/fig10_minimal_dynesty.png"))
    p.add_argument("--diag-csv", type=Path, default=Path("results/fig10_minimal_dynesty_diagnostics.csv"))
    p.add_argument("--samples-prefix", type=Path, default=Path("results/fig10_minimal_dynesty_samples.npz"))
    return p.parse_args()


def main() -> None:
    args = parse_args()
    rng = np.random.default_rng(args.seed)
    n_values = parse_n_values(args.n_values)

    wf_raw, info = load_sxs_waveform22(location=args.sxs_location, download=not args.no_download)
    wf, _ = align_to_peak(wf_raw)
    if info.remnant_mass is None or info.remnant_chif_z is None:
        raise ValueError("missing remnant metadata")

    t_all = wf.t
    h_all = wf.h
    h_peak_raw = float(np.max(np.abs(h_all)))
    if h_peak_raw <= 0:
        raise ValueError("invalid peak amplitude")

    amp_scale = float(args.target_hpeak / h_peak_raw)
    h_all = h_all * amp_scale

    m_sec = MSUN_SEC * args.m_total_msun
    ms_per_m = m_sec * 1e3
    delta_t0_m = float(args.delta_t0_ms / ms_per_m)
    t0 = 0.0 + delta_t0_m

    mask = (t_all >= t0) & (t_all <= args.t_end)
    t = t_all[mask]
    h = h_all[mask]
    if t.size < 64:
        raise ValueError("analysis window too short")
    tau_m = t - t0

    dt_m = float(np.median(np.diff(tau_m)))
    tau_u_m = np.arange(0.0, float(tau_m[-1]) + 0.5 * dt_m, dt_m)
    h_u = np.interp(tau_u_m, tau_m, h.real) + 1j * np.interp(tau_u_m, tau_m, h.imag)
    tau_u_sec = tau_u_m * m_sec

    freqs_hz = np.arange(args.f_min_hz, args.f_max_hz + 0.5 * args.df_hz, args.df_hz)
    psd = aligo_zero_det_high_power_psd(freqs_hz, f_low_hz=10.0)
    valid = (freqs_hz >= args.f_min_hz) & (freqs_hz <= args.f_max_hz) & np.isfinite(psd) & (psd > 0.0)

    signal_tilde = continuous_ft_from_time_series(tau_u_sec, h_u, freqs_hz)
    snr_before = optimal_snr(signal_tilde, psd, args.df_hz, valid_mask=valid)
    if snr_before <= 0:
        raise ValueError("invalid SNR before rescale")

    snr_rescale = float(args.target_snr_postpeak / snr_before)
    h_all = h_all * snr_rescale
    h_peak = float(np.max(np.abs(h_all)))
    h = h_all[mask]
    h_u = np.interp(tau_u_m, tau_m, h.real) + 1j * np.interp(tau_u_m, tau_m, h.imag)
    signal_tilde = continuous_ft_from_time_series(tau_u_sec, h_u, freqs_hz)
    snr_after = optimal_snr(signal_tilde, psd, args.df_hz, valid_mask=valid)

    if not np.isclose(snr_after, args.target_snr_postpeak, rtol=args.snr_rtol):
        raise ValueError(
            f"postpeak SNR={snr_after:.3f} is outside target {args.target_snr_postpeak:.3f} +/- {args.snr_rtol:.3f} rel"
        )

    noise_tilde = draw_colored_noise_rfft(
        rng,
        freqs_hz.size,
        psd,
        args.df_hz,
        enforce_real_endpoints=False,
    )
    d_tilde = signal_tilde + noise_tilde

    # Optional diagnostic perturbation: constant real bias in time-domain data.
    # This is intentionally injected into data only (not the model) to test sensitivity.
    bias_abs = 0.0
    if args.enable_constant_bias:
        bias_abs = float(args.constant_bias_rel * h_peak)
        bias_t = np.full_like(h_u, fill_value=bias_abs + 0.0j, dtype=complex)
        bias_tilde = continuous_ft_from_time_series(tau_u_sec, bias_t, freqs_hz)
        d_tilde = d_tilde + bias_tilde

    true_mf = float(info.remnant_mass * args.m_total_msun)
    true_chif = float(info.remnant_chif_z)
    mf_bounds = (args.mf_min_msun, args.mf_max_msun)
    chif_bounds = (args.chif_min, min(args.chif_max, 0.999))
    amp_bounds = (args.amp_min_rel, args.amp_max_rel)
    phi_bounds = (0.0, 2.0 * np.pi)

    colors = {0: "#1f77b4", 1: "#6f2da8", 2: "#d4a017", 3: "#d62728"}
    linestyles = {0: "-", 1: "--", 2: "--", 3: "-"}
    mf_grid = np.linspace(args.mf_min_msun, args.mf_max_msun, 200)
    chif_grid = np.linspace(args.chif_min, min(args.chif_max, 0.999), 200)

    pdf2d_by_n: dict[int, np.ndarray] = {}
    post_mf_by_n: dict[int, np.ndarray] = {}
    post_chif_by_n: dict[int, np.ndarray] = {}
    level90_by_n: dict[int, float] = {}

    diag_rows = [
        "N,ndim,niter,ncall_total,eff_percent,logz,logzerr,mf_q16,mf_q50,mf_q84,chif_q16,chif_q50,chif_q84"
    ]

    for n in n_values:
        model = RingdownInferenceModel(
            freqs_hz=freqs_hz,
            d_tilde=d_tilde,
            psd=psd,
            duration_sec=float(tau_u_sec[-1]),
            h_peak=h_peak,
            n_overtones=n,
            m_total_msun=args.m_total_msun,
            mf_bounds=mf_bounds,
            chif_bounds=chif_bounds,
            amp_bounds_rel=amp_bounds,
            phi_bounds=phi_bounds,
            qnm_chi_grid_size=args.qnm_chi_grid_size,
        )

        sampler = dynesty.DynamicNestedSampler(
            model.log_likelihood,
            model.prior_transform,
            model.ndim,
            nlive=args.nlive_init,
            bound="multi",
            sample=args.sample,
            walks=args.walks if args.sample == "rwalk" else None,
            slices=args.slices if args.sample in {"slice", "rslice", "hslice"} else None,
            rstate=np.random.default_rng(args.seed + 1000 * (n + 1)),
        )
        run_kwargs: dict[str, object] = dict(
            nlive_init=args.nlive_init,
            nlive_batch=args.nlive_batch,
            dlogz_init=args.dlogz_init,
            print_progress=False,
        )
        if args.maxbatch > 0:
            run_kwargs["maxbatch"] = args.maxbatch
        if args.maxcall > 0:
            run_kwargs["maxcall"] = args.maxcall
        if args.n_effective > 0:
            run_kwargs["n_effective"] = args.n_effective
        sampler.run_nested(**run_kwargs)
        res = sampler.results

        weights = np.exp(np.asarray(res.logwt, dtype=float) - float(res.logz[-1]))
        weights = weights / np.sum(weights)
        samples_eq = resample_equal(np.asarray(res.samples, dtype=float), weights)

        mf_s = samples_eq[:, 0]
        chif_s = samples_eq[:, 1]
        hist2d, _, _ = np.histogram2d(mf_s, chif_s, bins=[mf_grid, chif_grid], density=False)
        pdf2d = hist2d.T
        pdf2d = pdf2d / np.sum(pdf2d)
        pdf2d_by_n[n] = pdf2d
        level90_by_n[n] = credible_level_2d(pdf2d, cred=0.9)

        mf_centers = 0.5 * (mf_grid[:-1] + mf_grid[1:])
        chif_centers = 0.5 * (chif_grid[:-1] + chif_grid[1:])
        post_mf_by_n[n] = normalize_1d_pdf(mf_centers, np.histogram(mf_s, bins=mf_grid, density=False)[0].astype(float))
        post_chif_by_n[n] = normalize_1d_pdf(
            chif_centers,
            np.histogram(chif_s, bins=chif_grid, density=False)[0].astype(float),
        )

        eff_arr = np.asarray(res.eff).reshape(-1)
        eff_percent = float(eff_arr[-1]) if eff_arr.size > 0 else float("nan")
        ncall_total = int(np.sum(np.asarray(res.ncall)))
        niter = int(res.niter)
        logz = float(res.logz[-1])
        logzerr = float(res.logzerr[-1])
        mf_q16, mf_q50, mf_q84 = [float(v) for v in np.quantile(mf_s, [0.16, 0.5, 0.84])]
        ch_q16, ch_q50, ch_q84 = [float(v) for v in np.quantile(chif_s, [0.16, 0.5, 0.84])]
        diag_rows.append(
            f"{n},{model.ndim},{niter},{ncall_total},{eff_percent:.6f},{logz:.6f},{logzerr:.6f},"
            f"{mf_q16:.6f},{mf_q50:.6f},{mf_q84:.6f},{ch_q16:.6f},{ch_q50:.6f},{ch_q84:.6f}"
        )
        print(
            f"N={n} ndim={model.ndim} niter={niter} ncall={ncall_total} "
            f"logzerr={logzerr:.4f} mf_q50={mf_q50:.3f} chif_q50={ch_q50:.3f}"
        )

        if args.samples_prefix is not None:
            out_npz = n_tagged_path(args.samples_prefix, n)
            out_npz.parent.mkdir(parents=True, exist_ok=True)
            np.savez_compressed(out_npz, samples_equal=samples_eq, weights=weights)

    fig = plt.figure(figsize=(9, 8))
    gs = fig.add_gridspec(4, 4, hspace=0.05, wspace=0.05)
    ax_top = fig.add_subplot(gs[0, 0:3])
    ax_main = fig.add_subplot(gs[1:4, 0:3], sharex=ax_top)
    ax_right = fig.add_subplot(gs[1:4, 3], sharey=ax_main)

    mf_centers = 0.5 * (mf_grid[:-1] + mf_grid[1:])
    chif_centers = 0.5 * (chif_grid[:-1] + chif_grid[1:])
    handles: list[mlines.Line2D] = []
    for n in n_values:
        ax_main.contour(
            mf_centers,
            chif_centers,
            pdf2d_by_n[n],
            levels=[level90_by_n[n]],
            colors=[colors.get(n, "C0")],
            linestyles=[linestyles.get(n, "-")],
            linewidths=2.0,
        )
        ax_top.plot(mf_centers, post_mf_by_n[n], color=colors.get(n, "C0"), ls=linestyles.get(n, "-"), lw=1.8)
        ax_right.plot(post_chif_by_n[n], chif_centers, color=colors.get(n, "C0"), ls=linestyles.get(n, "-"), lw=1.8)
        handles.append(
            mlines.Line2D([], [], color=colors.get(n, "C0"), ls=linestyles.get(n, "-"), lw=2.0, label=f"N={n}")
        )

    ax_main.axvline(true_mf, color="k", ls=":", lw=1.2)
    ax_main.axhline(true_chif, color="k", ls=":", lw=1.2)
    ax_main.set_xlabel(r"$M_f\ [M_\odot]$")
    ax_main.set_ylabel(r"$\chi_f$")
    ax_main.set_xlim(args.mf_min_msun, args.mf_max_msun)
    ax_main.set_ylim(args.chif_min, min(args.chif_max, 0.999))
    ax_main.grid(True, alpha=0.15)
    ax_main.legend(handles=handles, loc="upper left", fontsize=9)

    ax_top.set_ylabel("Posterior")
    ax_top.tick_params(axis="x", labelbottom=False)
    ax_top.grid(True, alpha=0.15)
    ax_right.set_xlabel("Posterior")
    ax_right.tick_params(axis="y", labelleft=False)
    ax_right.grid(True, alpha=0.15)
    ax_top.set_title(rf"Minimal Fig.10 mapping with DynamicNestedSampler ($\Delta t_0={args.delta_t0_ms:.3f}$ ms)")
    fig.tight_layout()

    args.output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(args.output, dpi=180)

    args.diag_csv.parent.mkdir(parents=True, exist_ok=True)
    args.diag_csv.write_text("\n".join(diag_rows) + "\n", encoding="utf-8")

    print(f"source={wf.source}")
    print(f"distance_mpc={args.distance_mpc:.3f}")
    print(f"n_values={n_values}")
    print(f"true_mf_msun={true_mf:.6f}, true_chif={true_chif:.6f}")
    print(f"constant_bias_enabled={int(args.enable_constant_bias)}, constant_bias_rel={args.constant_bias_rel:.6e}, constant_bias_abs={bias_abs:.6e}")
    print(
        f"target_postpeak_snr={args.target_snr_postpeak:.3f}, "
        f"snr_before_rescale={snr_before:.3f}, snr_rescale_factor={snr_rescale:.6f}, snr_after_rescale={snr_after:.3f}"
    )
    print(f"diag_csv={args.diag_csv}")
    print(f"output={args.output}")


if __name__ == "__main__":
    main()
