...
def detector_strain_from_mode22(h22: np.ndarray) -> np.ndarray:
    """
    Build a real detector-strain proxy from complex h22.

    We apply a global phase rotation so the maximum-|h22| sample is real,
    then take the real part as h_det (plus-like channel).
    """
    if h22.ndim != 1:
        raise ValueError("h22 must be 1D")
    i_ref = int(np.argmax(np.abs(h22)))
    href = h22[i_ref]
    # 先做全局相位对齐，再取实部作为 detector-strain 代理通道。
...
def health_failed(acc: float, tau_mf: float, tau_chif: float, min_acceptance: float) -> tuple[bool, str]:
    ...
    # 严格门禁：低接受率或 tau 非有限值直接判失败，不生成发布图。
...
class RingdownPosterior:
    ...
    def sample_truth_ball(
        ...
        p = self.sample_prior(rng, nwalkers)
        p[:, 0] = np.clip(rng.normal(true_mf, mf_sigma, size=nwalkers), self.mf_bounds[0], self.mf_bounds[1])
        p[:, 1] = np.clip(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--verify-emcee-n-values", type=str, default="3")
    ...
    p.add_argument("--nwalkers", type=int, default=72)
    p.add_argument("--kombine-burnin", type=int, default=300)
    p.add_argument("--kombine-steps", type=int, default=1000)
    p.add_argument("--emcee-steps", type=int, default=1000)
    ...
    p.add_argument("--output", type=Path, default=Path("results/fig10_kombine_full.png"))
    p.add_argument("--diag-csv", type=Path, default=Path("results/fig10_kombine_full_diag.csv"))
    p.add_argument("--samples-prefix", type=Path, default=Path("results/fig10_kombine_full_samples.npz"))
    return p.parse_args()
...
def main() -> None:
    ...
    verify_n_values = set(parse_int_list(args.verify_emcee_n_values)) if args.verify_emcee_n_values.strip() else set()
    if any(n < 0 for n in n_values):
        raise ValueError("n-values must be nonnegative")
    if any(n not in n_values for n in verify_n_values):
        raise ValueError("verify-emcee-n-values must be a subset of n-values")
    ...
    if args.init_mode != "prior":
        raise ValueError("init_mode is forced to 'prior' for unbiased Fig.10 reproduction")
    ...
    # t_h-peak 口径：以 detector-strain 峰值定义环振起点参考时刻。
    ...
    # 把注入信号统一缩放到目标 post-peak SNR，保证不同运行可比。
    ...
    for n in n_values:
        ...
        p0 = model.sample_prior(rng, args.nwalkers)

        kombine = KombineSampler(args.nwalkers, model.ndim, model.log_posterior, processes=1)
        pos, lnpost, _ = kombine.run_mcmc(args.kombine_burnin, p0)
        pos, lnpost, _ = kombine.run_mcmc(args.kombine_steps, pos, lnpost0=lnpost)
        k_chain = np.asarray(kombine.chain[-args.kombine_steps :, :, :], dtype=float)
        k_flat = k_chain.reshape(-1, model.ndim)
        k_acc = float(np.mean(kombine.acceptance_fraction))
        k_tau_mf, k_tau_chi, k_ess = robust_tau_ess(k_chain)

        k_mf = k_flat[:, 0]
        k_chif = k_flat[:, 1]
        k_mf_q16, k_mf_q50, k_mf_q84 = [float(v) for v in np.quantile(k_mf, [0.16, 0.5, 0.84])]
        k_ch_q16, k_ch_q50, k_ch_q84 = [float(v) for v in np.quantile(k_chif, [0.16, 0.5, 0.84])]
        diag_rows.append(
            f"{n},kombine,{args.nwalkers},{args.kombine_burnin},{args.kombine_steps},{k_acc:.6f},"
            f"{k_tau_mf:.3f},{k_tau_chi:.3f},{k_ess:.2f},{k_mf_q16:.6f},{k_mf_q50:.6f},{k_mf_q84:.6f},"
            f"{k_ch_q16:.6f},{k_ch_q50:.6f},{k_ch_q84:.6f},{snr_after:.6f}"
        )
        ...
        if bad:
            ...
            raise RuntimeError(f"N={n} kombine health check failed: {reason}")

        # emcee verification for selected N values
        if n in verify_n_values and args.emcee_steps > 0:
            em = emcee.EnsembleSampler(args.nwalkers, model.ndim, model.log_posterior)
            em.run_mcmc(pos, args.emcee_steps, progress=False)
            e_chain = em.get_chain()
            e_flat = em.get_chain(flat=True)
            e_acc = float(np.mean(em.acceptance_fraction))
            e_tau_mf, e_tau_chi, e_ess = robust_tau_ess(e_chain)
            e_mf = e_flat[:, 0]
            e_chif = e_flat[:, 1]
            e_mf_q16, e_mf_q50, e_mf_q84 = [float(v) for v in np.quantile(e_mf, [0.16, 0.5, 0.84])]
            e_ch_q16, e_ch_q50, e_ch_q84 = [float(v) for v in np.quantile(e_chif, [0.16, 0.5, 0.84])]
            diag_rows.append(
                f"{n},emcee,{args.nwalkers},0,{args.emcee_steps},{e_acc:.6f},"
                f"{e_tau_mf:.3f},{e_tau_chi:.3f},{e_ess:.2f},{e_mf_q16:.6f},{e_mf_q50:.6f},{e_mf_q84:.6f},"
                f"{e_ch_q16:.6f},{e_ch_q50:.6f},{e_ch_q84:.6f},{snr_after:.6f}"
            )
        ...
        hist2d, _, _ = np.histogram2d(k_mf, k_chif, bins=[mf_grid, chif_grid], density=False)
        pdf2d = hist2d.T
        if np.sum(pdf2d) <= 0:
        ...
        post_mf_by_n[n] = normalize_1d_pdf(mf_centers, np.histogram(k_mf, bins=mf_grid, density=False)[0].astype(float))
        post_chif_by_n[n] = normalize_1d_pdf(
            chif_centers,
            np.histogram(k_chif, bins=chif_grid, density=False)[0].astype(float),
        )

        if args.samples_prefix is not None:
            out_k = args.samples_prefix.with_name(args.samples_prefix.stem + f"_N{n}_kombine.npz")
            np.savez_compressed(out_k, chain=k_chain, flat=k_flat)
            if e_flat is not None and e_chain is not None:
        ...
        print(
            f"N={n} kombine_acc={k_acc:.4f} mf_q50={k_mf_q50:.3f} chif_q50={k_ch_q50:.3f}"
            + (f" | emcee_verified={'yes' if n in verify_n_values else 'no'}" if args.emcee_steps > 0 else "")
        )

    # Fig10-style combined figure (kombine posteriors for N list)
    ...
    ax_top.set_title(rf"Fig.10-style posteriors with kombine ($\Delta t_0={args.delta_t0_ms:.3f}$ ms)")
    fig.tight_layout()
    ...
    print(f"n_values={n_values}, verify_emcee_n_values={sorted(verify_n_values)}")
